...
class employee_attendance_controller extends Controller
{
    ...
    {
        $employee = Employee::select('id','en_first_name','en_middle_name','en_last_name')->toArray();
        // dd($employee);
        $data = array();
        ...
        for ($i=$date_from; $i<=$date_to; $i+=86400) {  
            ...
            dd($data);
        Excel::create('Attendance_Cheet', function ($excel) use ($data,$employee) {
            $excel->sheet('Attendance', function ($sheet) use ($data,$employee) {
                $sheet->loadView('admin.employee.attendance_cheet',['dateArray',$data,$employee]);
            });
        })->export('xls');
    }
}
