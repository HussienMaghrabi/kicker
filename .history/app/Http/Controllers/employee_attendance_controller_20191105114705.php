...
class employee_attendance_controller extends Controller
{
    ...
    public function export_xsl(Request $request)
    {
        ...
        $countEmployee = '';
        $countDate = '';
        $employee = Employee::select('id','en_first_name','en_middle_name','en_last_name')->get();
        dd($employee);
        ...
        Excel::create('Attendance_Cheet', function ($excel) use ($data,$employee,$countDate,$countEmployee) {
            $excel->sheet('Attendance', function ($sheet) use ($data,$employee,$countDate,$countEmployee) {
                $sheet->loadView('admin.employee.attendance_cheet',['dateArray' => $data,'employees' => $employee , 'countDate' => $countDate , 'countEmployee' => $countEmployee]);
            });
        })->export('xls');
    }
}
