<template>

    <div class="container">
        <div class="card">
            <header class="card-header level ">
                <div class="level">
                    <div class="level-item">
                        <p class="card-header-title">
                            Phase Info
                        </p>
                    </div>
                </div>
            </header>
            <div class="card-content">
                <div class="column first">
                    <div class="columns is-multiline is-mobile" style="margin-bottom:20px">
                        <div class="column is-12">
                            <b><label>ID: </label></b>
                            <span>{{ phase.id }}</span>
                            <hr>
                        </div>
                        <div class="column is-6">
                            <b><label>English Name: </label></b>
                            <span>{{ phase.en_name }}</span>
                            <hr>
                        </div>
                        <div class="column is-6">
                            <b><label>Arabic Name: </label></b>
                            <span>{{ phase.ar_name }}</span>
                            <hr>
                        </div>
                        <div class="column is-12">
                            <b><label>English Description: </label></b>
                            <span>{{ phase.en_description }}</span>
                            <hr>
                        </div>
                        <div class="column is-12">
                            <b><label>Arabic Description: </label></b>
                            <span>{{ phase.ar_description }}</span>
                            <hr>
                        </div>
                        <div class="column is-4">
                            <b><label>Price / Meter : </label></b>
                            <span>{{ phase.meter_price }}</span>
                            <hr>
                        </div>
                        <div class="column is-4">
                            <b><label>Area : </label></b>
                            <span>{{ phase.area }} </span>
                            <hr>
                        </div>
                        <div class="column is-4">
                            <b><label>Delivery Date : </label></b>
                            <span>{{ phase.delivery_date }}</span>
                            <hr>
                        </div>
                        <div class="column is-12">
                            <b><label>Facilities : </label></b>
                            <span class="facilities" v-for="facility in facilities">{{ facility.name }}</span>
                            <hr>
                        </div>

                        <div class="column is-12">
                            
                            <router-link :to="{ name: 'createProperty', params: { phase: phase }}"
                             class=" button is-success">Add Property</router-link>
                        </div>
                        <div class="table_wrapper">
                            <b-table
                            :data="properties"
                            :paginated="isPaginated"
                            :per-page="perPage"
                            :default-sort-direction="defaultSortDirection"
                            default-sort="user.first_name"
                            aria-next-label="Next page"
                            aria-previous-label="Previous page"
                            aria-page-label="Page"
                            aria-current-label="Current page"
                            id="table">
                            
                            <template slot-scope="props">
                                <b-table-column style="float:left;" field="id" label="ID" width="40" sortable numeric>
                                    {{ props.row.id }}
                                </b-table-column>

                                <b-table-column class="width" field="name" label="Name" sortable>
                                    {{ props.row.name }}
                                </b-table-column>

                                <b-table-column class="width" field="type" label="Type" sortable>
                                    {{ props.row.type }}
                                </b-table-column>

                                <b-table-column class="width" field="start_price" label="Price" sortable>
                                    {{ props.row.start_price }}
                                </b-table-column>

                                <b-table-column class="padding" field="" label="show" sortable >
                                    <router-link class="button is-primary" :to="'/admin/vue/showProperty/'+props.row.id" style="color:#fff;">
                                        Show
                                    </router-link>  
                                </b-table-column>

                                <b-table-column class="padding btn_delete" field="phase_id" label="delete" sortable>
                                    <button class=" button is-danger" @click="deletePropertyActive(props.row)">
                                            Delete  
                                    </button>
                                </b-table-column>

                            </template>
                            <template slot="empty" v-if="!isLoading && isEmpty">
                                        <section class="section">
                                            <div class="content has-text-grey has-text-centered">
                                                <p>
                                                    <b-icon
                                                    icon="emoticon-sad"
                                                    size="is-large">
                                                </b-icon>
                                            </p>
                                            <p>Nothing here.</p>
                                        </div>
                                    </section>
                                    <hr>
                                </template>
                            </b-table>
                        </div>
                    </div>
                </div>
                <b-loading :is-full-page="isFullPage" :active.sync="isLoading" :can-cancel="true"></b-loading>
            </div>
        </div>
    </div>
</template>
<script>

import {getPhase,getProjectPhotos,deleteProperty} from './../../calls'

export default {
     data() {
            return {
                phase: {},
                location: '',
                isLoading: true,
                token: window.auth_user.csrf,
                userType: window.auth_user.type,
                id: null,
                facilities: [],
                properties: [],
                propertiesCurrentNumber: 0,
                propertiesTotalNumber: 0,
                isEmpty: false,
                isPaginated: true,
                isPaginationSimple: true,
                defaultSortDirection: 'desc',
                page: 1,
                perPage: 10,
                isFullPage: true,
                paginationPosition: 'bottom',
                currentPage: 1,
            }
        },
        created() {
            this.id = this.$route.params.id
        },
        mounted() {
            this.getData()
            $('#table > div.table-wrapper > table')
            .css({"background-color": "#f3f1f1fa", "border": "1px solid #e2d1d1","border-radius":"10px"});
        },
        components: {

        },
        methods: {
            getData(){
                this.isLoading = true
                getPhase(this.id).then(response=>{
                    console.log(response)
                    this.phase = response.data.phase
                    this.facilities = response.data.facilities
                    this.properties = response.data.properties
                    // this.phasesCurrentNumber = Math.min(response.data.total,this.page * this.perPage)
                    // this.phasesTotalNumber = response.data.total
                    // if(this.phases.length == 0){
                    //     this.isEmpty = true
                    // }
                    this.isLoading = false
                })
                .catch(error => {
                    console.log(error)
                })
            },
            errorDialog(msg) {
                this.$dialog.alert({
                    title: 'Error',
                    message: msg,
                    type: 'is-danger',
                })
            },
            success(action) {
                this.$toast.open({
                    message: 'Property '+action+' Successfully',
                    type: 'is-success',
                    position: 'is-bottom',
                    duration: 5000,
                })
            },
            deletePropertyActive(property) {
                this.$dialog.confirm({
                    title: 'Deleting Property',
                    message: 'Are you sure you want to <b>delete</b> '+property.name+ ' ?',
                    confirmText: 'Delete Property',
                    type: 'is-danger',
                    hasIcon: true,
                    onConfirm: () => this.deleteProperty(property.id)
                })
            },
            deleteProperty(propertyID){
                deleteProperty(propertyID).then(response => {
                    this.success('Deleted')
                    this.getData()
                })
                .catch(error => {
                    this.errorDialog('Something went wrong please try again later')
                    console.log(error)
                })
            },
        }
}
</script>

<style scoped>
    .facilities{
        background-color: #7484fd;
        border: 1px solid #4c5ee6;
        border-radius: 15px;
        color: #FFFFFF;
        padding: 4px;
        margin-right: 6px;
        margin-left: 6px;
    }
    #logo,#cover{
        padding: 10px;
    }
    #logo img,#cover img{
        width: 100%;
        height: 500px;
    }
    .table_wrapper{
        width: 100%;
    }
</style>
