...
<script>
import {getTodayLeads,deleteLead,switchLeads,searchForLead,getAgents,checkUserGroupAndRoles} from './../../calls'
    ...
    export default {
        ...
        methods: {
            ...
            getData(scrollSwitch = false){
                this.isLoading = true
                getTodayLeads(this.page).then(response=>{
            ...
            onPageChange(page) {
                this.page = page
                this.$router.replace({ name: "todayleads", params: {page: page} })
                this.getData()
            },
            success(action) {
                this.$toast.open({
