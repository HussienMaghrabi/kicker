...
<script>
import {getRotationalLeads,deleteLead,switchLeads,getAgents,checkUserGroupAndRoles,searchForLead} from './../../calls'
    ...
    export default {
        ...
        methods: {
            ...
            getData(scrollSwitch = false){
                this.isLoading = true
                getRotationalLeads(this.page).then(response=>{
            ...
            onPageChange(page) {
                this.page = page
                this.$router.replace({ name: "RotationalLeads", params: {page: page} })
                this.getData()
            },
            success(action) {
                this.$toast.open({
