...
<script>
import {getFavoriteLeads,deleteLead,switchLeads,getAgents,checkUserGroupAndRoles,searchForLead} from './../../calls'
    ...
    export default {
        ...
        methods: {
            ...
            getData(scrollSwitch = false){
                this.isLoading = true
                getFavoriteLeads(this.page).then(response=>{
            ...
            onPageChange(page) {
                this.page = page
                this.$router.replace({ name: "FavoriteLeads", params: {page: page} })
                this.getData()
            },
            success(action) {
                this.$toast.open({
