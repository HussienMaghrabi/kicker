...
<script>
import {getHotLeads,deleteLead,switchLeads,searchForLead,getAgents,checkUserGroupAndRoles} from './../../calls'
    ...
    export default {
        ...
        methods: {
            ...
            getData(scrollSwitch = false){
                this.isLoading = true
                getHotLeads(this.page).then(response=>{
            ...
            onPageChange(page) {
                this.page = page
                this.$router.replace({ name: "HotLeads", params: {page: page} })
                this.getData()
            },
            success(action) {
                this.$toast.open({
