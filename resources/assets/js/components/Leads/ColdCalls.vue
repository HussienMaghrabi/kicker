      ...
      <template slot-scope="props">
        ...
        <b-table-column field="favorite" label="Favorite" centered sortable>
          <i
            ...
            @click="changeLeadStatus('fav',props.row.id)"
          ></i>
          <i
            class="fa fa-star"
            aria-hidden="true"
        ...
        <b-table-column field="hot" label="Hot" centered sortable>
          <i
            ...
            style="color: #caa42d"
            v-if="props.row.hot"
            @click="changeLeadStatus('hot',props.row.id)"
          ></i>
          <i
            class="fa fa-fire"
            aria-hidden="true"
  ...
  methods: {
    ...
    getPublic(){
      getPublicData().then(response=>{
          ...
          for(var i=0;i<this.callStatus.length;i++){
            if(this.callStatus[i]['id'] == "6"){
              this.callStatus.splice(i, 1)
              break;
            }
          }
    ...
    onPageChange(page) {
      this.page = page;
      this.$router.replace({ name: "ColdCalls", params: { page: page } });
      this.getData();
    },
    success(action) {
      this.$toast.open({
