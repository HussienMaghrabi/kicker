...
<section class="container jobs">
    ...
    <div class="add">
         <button name="add" class="btnAdd">
            <router-link :to="'/admin/vue/AddjobTitle'" style="color:#fff;"> 
                    Add
            </router-link>
        </button><br>
        <div class="search">
            <label>Search: </label>
            <b-input @input="filterJobTitle" v-model="search_query" type="text" class="inputSearch"></b-input>
        </div>
    </div>
            ...
            <template slot-scope="props">
                ...
                <b-table-column label="Update">
                    <b-button type="is-warning">
                         ...
                         <router-link :to="'/admin/vue/editJobTitle/'+props.row.id" style="color:#000;">
                            Update
                        </router-link>
...
import {allJobTitles,filterJobTitle,deleteThisJobTitle} from './../../calls'
export default {
    data() {
            return {
                jobTitleName:null,
        ...
        created() {
         ...
         methods:{
            ...
            this.isLoading = true
                ...
                allJobTitles(this.page).then(response=>{
       ...
       DeleteFromIndex(id) {
        //    alert(id)
                this.$dialog.confirm({
                    title: 'Deleting ',
                    message: 'Are you sure you want to <b>delete</b> Job Title?',
                    confirmText: 'Delete',
                    type: 'is-danger',
                    hasIcon: true,
                    onConfirm: () => this.deleteJobTitle(id)
                })
            },
        deleteJobTitle(id){
                deleteThisJobTitle(id).then(response=>{
        ...
        this.$dialog.alert({
            title: 'Error',
            message: 'Please select the Job Titleyou want to delete first',
            type: 'is-danger',
            })
        },
        success(action) {
        ...
        filterJobTitle(data).then(response=>{
