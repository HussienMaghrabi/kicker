...
<section class="apps container">           
    ...
    <div class="add">
        <button name="add" class="btnAdd">
            <router-link :to="'/admin/vue/addEmployee'" style="color:#fff;"> 
                    Add
            </router-link>
        </button><br>
        <div class="search">
            <label>Search: </label>
            <b-input @input="filterEmployees" v-model="search_query" type="text" class="inputSearch"></b-input>
        </div>
    </div>
...
export default {
    data() {
        ...
        onPageChange(page) {
                // console.log('sdawd',page)
                this.page = page
                this.$router.replace({ name: "employees", params: {page: page} })
                this.getData()
            },
        DeleteFromIndex(id) {
                this.$dialog.confirm({
