...
@section('content')
    <div class="box">
        ...
        <div class="box-body">
                ...
                <select name="lead_id" class="form-control select2" style="width: 100%"
                    ...
                    @foreach(@App\Lead::getAgentLeads() as $lead)
                        <option value="{{ $lead->id }}"
                                ...
                                @elseif(request()->lead == $lead->id) selected
                                @elseif($call->lead_id == $lead->id) selected @endif>
            ...
            <div class="form-group @if($errors->has('call_status_id')) has-error @endif" style="width:97%">
                <label>{{ trans('admin.call_status') }}</label>
                <select class="form-control select2" name="call_status_id" data-placeholder="{{ __('admin.call_status_id') }}">
                    <option></option>
                    @foreach(@\App\CallStatus::get() as $status)
                        <option value="{{ $status->id }}" next="{{ $status->has_next_action }}" @if($call->call_status_id == $status->id) selected @endif>
                            {{$status->name}}
                        </option>
                    @endforeach
                </select>
