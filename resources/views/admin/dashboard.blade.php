            ...
            <form action="{{ url(adminPath().'/tasks') }}" method="post">
                <div class="modal-body">
                    ...
                    <div class="form-group @if($errors->has('agent_id')) has-error @endif">
                        ...
                        <select class="form-control" data-placeholder="{{ trans('admin.agent') }}" name="agent" style="width: 100%">
        ...
        <div class="row" width="100%">
            ...
            <div id="sortableDesk" class="col-md-8">
                ...
                <div n="1" id="r1" class="ui-state-default dash_desk">
                    <span class="pull-right" style="margin-right: 30px; font-size: 30px;">{{ date("Y-m-d") }}</span>
                    <label>
                        <h2>
                            <img src="{{url('icon/leads-sources.png')}}"/> Follow UP
                        </h2>
                    </label>
                    <div>
                        <ul class="admin-noti" style="list-style: none; height: 300px; overflow-y: scroll;">
                            @php
                            $tasks = \App\Task::where( 'user_id', auth()->user()->id )->whereDate('created_at',  date("Y-m-d"))->where('status',  'pending')->get();
                            @endphp
                            @foreach($tasks as $task)
                                @php
                                    $newArray = json_decode($task->leads);
                                @endphp
                                @foreach($newArray as $lead)
                                    <li>
                                        <h3>
                                            <img src="{{url('icon/call.png')}}" style="width:25px" />
                                            {{ \App\Lead::find($lead)->first_name.' '. \App\Lead::find($lead)->last_name }}
                                            <span>{{ date('d/m/y --- h:i:s a', strtotime($task->created_at)) }}</span>
                                            <a class="btn btn-primary pull-right" href="/admin/confirm_to_do/{{$task->id}}" style="color: white">Done</a>
                                        </h3>
                                    </li>
                                @endforeach
                            @endforeach
                            ...
                            @php
                            $toDos = \App\ToDo::where( 'user_id', auth()->user()->id )->whereDate('created_at', date("Y-m-d"))->where('status',  'pending')->get();
                            //dd($toDos);
                            @endphp
                           @foreach($toDos as $toDo)
                                <li>
                                    <h3>
                                        ...
                                        <span>{{ date('d/m/y --- h:i:s a', strtotime($toDo->created_at)) }}</span> <br >
                                        <span>phone::  {{ \App\Lead::find($toDo->leads)->phone }}</span>
                                        <!-- Large modal -->
                                        <button type="button" class="btn btn-primary pull-right" data-toggle="modal" data-target="#modal-{{$toDo->id}}">Done</button>
                                               <!-- Modal -->
                                        @if($toDo->to_do_type == 'call')
                                        <div class="modal fade bs-example-modal-lg" id="modal-{{$toDo->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                          <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                              <div class="modal-header">
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                                <h4 class="modal-title" id="myModalLabel">Add Call</h4>
                                              </div>
                                              <div class="modal-body">
                                                {!! Form::open(['url' => adminPath().'/calls' ,  'style'=>'width: 100%']) !!}
                                                {{-- <form  method="post"> --}}
                                                    @php
                                                        $show = \App\Lead::find($toDo->leads);
                                                    @endphp
                                                ...
                                                <div class="form-group @if($errors->has('contact_id')) has-error @endif" id="contact">
                                                    ...
                                                    <select name="contact_id" class="form-control select2" id="contact_id" style="width: 100% !important"
                                                ...
                                                <div class="form-group @if($errors->has('phone')) has-error @endif">
                                                    ...
                                                    <select name="phone" class="form-control select2" id="phone" style="width: 100% !important" data-placeholder="{{ trans('admin.phone') }}">
                                                    <option value="{{ @$show->phone }}">{{ @$show->phone }}</option>
                                                        @if(@$lead->other_phones != null)
                                                            @php
                                                                $allPhones = json_decode(@$lead->other_phones);
                                                            @endphp
                                                ...
                                                <div class="form-group @if($errors->has('call_status_id')) has-error @endif">
                                                    ...
                                                    <select class="form-control select2" name="call_status_id" id="callStatus" data-placeholder="{{ __('admin.call_status') }}" style="width: 100% !important" required="">
                                                        <option></option>
                                                ...
                                                <div class="form-group @if($errors->has('probability')) has-error @endif">
                                                    ...
                                                    <select class="form-control select2" id="callprobability" name="probability" style="width: 100% !important" data-placeholder="{{ __('admin.probability') }}">
                                                        <option></option>
                                                ...
                                                <div class="form-group @if($errors->has('projects')) has-error @endif">
                                                    ...
                                                    <select multiple id="callprojects" class="form-control select2" name="projects[]" style="width: 100% !important"
               ...
               <div n="1" id="r1" class="ui-state-default dash_desk">
                    <label><h2><img src="{{url('icon/dashboard.png')}}"/>DASHBOARD</h2></label>
                    <div>
                        <div class="d-inline">
                        ...
                        @endif
                        <div class="d-inline">
                            <p>{{ trans('admin.today_leads') }}</p>
                            <h3 style="color:#E12F4F"><a href="/admin/leads#/todayleads/1">{{ $todayLeads }}</a></h3>
                            <span>Today Leads</span>
                        </div>

                        <div class="d-inline">
                            <p>{{ trans('admin.seen') }}</p>
                            <h3 style="color:#E12F4F"><a href="/admin/leads#/seen/1">{{ $seen }}</a></h3>
                            <span>Seen</span>
                        </div>

                        <div class="d-inline">
                            <p>{{ trans('admin.notSeen') }}</p>
                            <h3 style="color:#E12F4F"><a href="/admin/leads#/notseen/1">{{ $notSeen }}</a></h3>
                            <span>Not Seen</span>
                        </div>
                        <br>
            ...
            <div class="col-md-4 activityLog" style="background:#F6ECE2;">
                ...
                @if(auth()->user()->type=='admin' || count(\App\Group::where('team_leader_id',auth()->user()->id)->get())>0)
                    <label>
                        <h2><img src="{{url('icon/Activity-log.png')}}"/>Activity Log</h2>
                    </label>
        ...
        @foreach($groups as $group)
            @if($group->team_leader_id==auth()->user()->id)
            {
                ...
                data:[
                            ...
                            {
                            title:"{{$member->name}}",href:"javascript:void(null)",dataAttrs:[
                                {title:"parent",data:"{{$member->group_id}}"},
                                {title:"user_id",data:"{{$member->id}}"},
                                ],
                            },
                            @endforeach
                        ]
            ...
            @if(auth()->user()->type=='admin' || count(\App\Group::where('team_leader_id',auth()->user()->id)->get())>0)
                ...
                dayClick: function (date, allDay, jsEvent, view) {
                    ...
                    if (days >= 0) {
                        ...
                        $('select[name="agent"]').removeAttr('readonly');
            ...
            $('#calendar2').fullCalendar({
                ...
                dayClick: function (date, allDay, jsEvent, view) {
                    ...
                    if (days >= 0) {
                        ...
                        $('#addEvent').modal('show');
                        $('select[name="agent"]').attr('readonly','');
                        $('#newDate').html(date.format('YYYY-M-D'));
