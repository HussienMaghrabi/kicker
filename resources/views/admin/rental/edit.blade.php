                    ...
                    <select class="form-control select2" data-placeholder="{{ trans('admin.lead') }}" name="lead_id">
                        ...
                        @foreach($agentLeads as $lead)
                            <option value="{{ $lead->id }}"
                                ...
                                -
                                @if($lead->agent_id == auth()->id() || $lead->commercial_agent_id == auth()->id())
                                    {{ __('admin.my_lead') }}
                                @else
                                    {{ __('admin.team_lead', ['agent' => $lead->agentName]) }}
                                @endif
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="box-body">
                ...
                <div class="col-md-12">
                    @if(count($images)>0)
                        ...
                        <div class="col-md-3" id="image_parent{{ $image->id }}">
                            <img class="img-thumbnail" src="{{ url(''.$image->image) }}">
                            <button class="btn btn-danger image_id" data="{{ $image->id }}" >Delete</button>
                        </div>
                        @endforeach
                    @endif
                </div>
