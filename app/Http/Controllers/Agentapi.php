...
class Agentapi extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        $user = User::where('email', $request->input('email'))->where(
            function ($q) {
                return $q->where('is_locked', '<>', 1)->orWhere('is_locked', '=', null);
            }
        )->first();
        if ($user) {
            ...
            if ($check) {
                ...
                if (count(Player::where('player_id', $request->player_id)->get()) < 1) {
                $player = new Player;
                $player->user_id = $user->id;
                $player->player_id = $request->player_id;
                $player->device = $request->device;
                $player->status = 1;
                $player->save();
                }
                ...
                if ($user->flag == 1) { // must change password
                    return ["status" => 'error', "msg" => 'You need to change your password'];
                }
                $response = array("status" => 'ok', "msg" => 'success', "id" => $user->id, "name" => $user->name, "title" => $title, "image" => $user->image, "email" => $user->email, "phone" => $user->phone, "token" => $token, "type" => $type, "cil" => @$cil, "calls" => @$calls, "meetings" => @$meetings, "requests" => @$requests, "edit_leads" => @$edit_leads, "projects" => @$projects, "show_resale" => @$show_resale, "show_rental" => @$show_rental, "add_rental" => @$add_rental, "add_resale" => @$add_resale, "admin" => @$admin);
                return $response;
            } else {
                return ['status' => 'error', "msg" => "can't login"];
            }
        } else {
    ...
    public function requestExecuse(Request $request)
    {
        $rules = [
            'reason' => 'required',
        ...
        if ($validator->fails()) {
            ...
            $execuse = new TimeExecuse();
            $execuse->employee_id = $employee_id;
            $execuse->type = $request->type;
            $execuse->reason = $request->reason;
            $execuse->is_approved = -1;
            $execuse->number_of_hours = $request->number_of_hours;
            $execuse->date = $request->date;
            $execuse->save();
            return ['status' => 'ok'];
        }
    }
    ...
    public function list_broadcastrequests(Request $req)
    {
        //$request = json_decode($req->getContent());
        $result = \App\RequestBroadcast::with(['unittype', 'loc', 'project', 'user', 'lead'])->get();
        return ['status' => 'ok', 'requests' => $result];
    }

    public function single_broadcastrequests(Request $req)
    {
        $request = json_decode($req->getContent());
        $id = $request->id;
        $result = \App\RequestBroadcast::with(['unittype', 'loc', 'project', 'user', 'lead'])->find($id);
    ...
    public function lead_action(Request $request)
    {
        ...
        DB::table('lead_actions')->insert(['lead_id' => $request->lead_id, 'type' => $request->type, 'agent_type' => $agent_type, 'time' => strtotime($request->time), 'user_id' => $request->user_id]);
    ...
    public function events(Request $request)
    {
        ...
        // dd((int)substr(strtotime($date), 0, 5));
        $date = (int)substr(strtotime($date), 0, 5);
        $meeting = ToDo::where('user_id', $user_id)->where('due_date',"like",$date."%")->where('to_do_type', 'meeting')->where('status', 'pending')->get();
        $calls = ToDo::where('user_id', $user_id)->where('due_date',"like",$date."%")->where('to_do_type', 'call')->where('status', 'pending')->get();
        // dd(date("Y-m-d", 1539937500));
        // dd($meeting);
        $others = ToDo::where('user_id', $user_id)->where('due_date', $date)->where('to_do_type', 'others')->where('status', 'pending')->get();
        // dump($others);
        $meeting1 = Task::where('user_id', $user_id)->where('due_date', "like",$date."%")->where('task_type', 'meeting')->where('status', 'pending')->get();
        // dump($meeting1);
        $calls1 = Task::where('user_id', $user_id)->where('due_date', "like",$date."%")->where('task_type', 'call')->where('status', 'pending')->get();
        ...
        foreach ($meeting as $row) {
            ...
            if ($lang == 'ar') {
                array_push($data['meeting'], array('name' => $lead, 'type' => 'todo', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '', 'projects' => Project::whereIn('id', array(1, 2, 3))->select('id', 'ar_name as name')->get()->toArray()));
            } else if ($lang == 'en') {
                array_push($data['meeting'], array('name' => $lead, 'type' => 'todo', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '',
        ...
        foreach ($meeting1 as $row) {
            ...
            $phone = $row->phone ? $row->phone : @$leadData->phone;
            if ($lang == 'ar') {
                array_push($data['meeting'], array('name' => $lead, 'type' => 'task', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '', 'projects' => Project::whereIn('id', $dd)->select('id', 'ar_name as name')->get()->toArray()));
            } else if ($lang == 'en') {
                array_push($data['meeting'], array('name' => $lead, 'type' => 'task', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '',
                    // 'projects' => Project::whereIn('id', $dd)->select('id', 'en_name as name')->get()->toArray()
                ));
            }
            $cm++;
        ...
        foreach ($calls as $row) {
            ...
            $phone = $row->phone ? $row->phone : @$leadData->phone;
            if ($lang == 'ar') {
                array_push($data['calling'], array('name' => $lead, 'type' => 'todo', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $row->phone ? $row->phone : @$leadData->phone, 'description' => $row->description ? $row->description : '',
                    // 'projects' => Project::whereIn('id', $dd)->select('id', 'ar_name as name')->get()->toArray()
                ));
            } else if ($lang == 'en') {
                array_push($data['calling'], array('name' => $lead, 'type' => 'todo', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '',
                    // 'projects' => Project::whereIn('id', $dd)->select('id', 'en_name as name')->get()->toArray()
                ));
            }
            $cc++;
        ...
        foreach ($calls1 as $row) {
            ...
            $phone = $row->phone ? $row->phone : @$leadData->phone;
            if ($lang == 'ar') {
                array_push($data['calling'], array('name' => $lead, 'type' => 'task', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? $row->time : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '', 'projects' => Project::whereIn('id', $dd)->select('id', 'ar_name as name')->get()->toArray()));
            } else if ($lang == 'en') {
                $phone = '';
                $phone = $row->phone ? $row->phone : @$leadData->phone;
                array_push($data['calling'], array('name' => $lead, 'type' => 'task', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '',
                    // 'projects' => Project::whereIn('id', $dd)->select('id', 'en_name as name')->get()->toArray()
                ));
            }
            $cc++;
        ...
        foreach ($others as $row) {
            ...
            $phone = $row->phone ? $row->phone : @$leadData->phone;
            if ($lang == 'ar') {
                array_push($data['others'], array('name' => $lead, 'type' => 'todo', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '',
                    // 'projects' => Project::whereIn('id', $dd)->select('id', 'ar_name as name')->get()->toArray()
                ));
            } else if ($lang == 'en') {
                array_push($data['others'], array('name' => $lead, 'type' => 'todo', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '',
        ...
        foreach ($others1 as $row) {
            ...
            $phone = $row->phone ? $row->phone : @$leadData->phone;
            if ($lang == 'ar') {
                array_push($data['others'], array('name' => $lead, 'type' => 'task', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '',
                    // 'projects' => Project::whereIn('id', $dd)->select('id', 'ar_name as name')->get()->toArray()
                ));
            } else if ($lang == 'en') {
                array_push($data['others'], array('name' => $lead, 'type' => 'task', 'id' => $row->id, 'date' => date('Y-m-d', $row->due_date), 'location' => $row->location ? $row->location : '', 'time' => $row->time ? date('H:i', $row->time) : '', 'phone' => $phone, 'description' => $row->description ? $row->description : '', 'projects' => Project::whereIn('id', $dd)->select('id', 'en_name as name')->get()->toArray()));
            }
            $co++;
        }
        ...
        if ($target == null) {
            ...
            $calls = @\App\Call::where('user_id', $request->user_id)->where('created_at', '>=', $target->month . '-01 00:00:00')->where('created_at', '<=', $target->month . '-31 23:59:59')->count();
            if ($target->calls) {
                $callsPercent = $calls * 100 / $target->calls;
            } else {
                $callsPercent = 0;
            }
            $meetings = @\App\Meeting::where('user_id', $request->user_id)->where('created_at', '>=', $target->month . '-01 00:00:00')->where('created_at', '<=', $target->month . '-31 23:59:59')->count();
            if ($target->meetings) {
                $meetingsPercent = $meetings * 100 / $target->meetings;
            } else {
                $meetingsPercent = 0;
            }
            $leads = @\App\Lead::where('agent_id', $request->user_id)->where('created_at', '>=', $target->month . '-01 00:00:00')->where('created_at', '<=', $target->month . '-31 23:59:59')->count();
            if ($target->leads) {
                $leadsPercent = $leads * 100 / $target->leads;
            } else {
                $leadsPercent = 0;
            }
            $money = @\App\ClosedDeal::where('agent_id', $request->user_id)->where('created_at', '>=', $target->month . '-01 00:00:00')->where('created_at', '<=', $target->month . '-31 23:59:59')->sum('price');
            if ($target->money) {
        ...
        $data['target'] = array('calls' => $calls, 'all_calls' => $target->calls, 'call_percent' => $callsPercent, 'meeting' => $meetings, 'all_meeting' => $target->meetings, 'meeting_percent' => $meetingsPercent, 'leads' => $leads, 'all_leads' => $target->leads ? $target->leads : 0, 'lead_percent' => $leadsPercent, 'money' => $money, 'all_money' => $target->money, 'money_percent' => $moneyPercent, 'call_count' => $cc, 'meeting_count' => $cm, 'other_count' => $co);
        //////////////////////////////////////////
        foreach ($todos as $row) {
            array_push($data['calendar'], array('date' => date('Y-m-d', $row->due_date), 'type' => $row->to_do_type));
        }
        foreach ($tasks as $row) {
            array_push($data['calendar'], array('date' => date('Y-m-d', $row->due_date), 'type' => $row->task_type));
        }
    ...
    public function profile(Request $request)
    {
        ...
        $ytarget = Target::where('agent_type_id', $agent_type_id)->where('month', '>=', $thisyear)->where('month', '<=', $nextyear)->sum('money');
    ...
    public function mission_complete(Request $request)
    {
        ...
        foreach ($requests->data as $request) {
            $lead_id = 0;
            $rules = ['probability' => 'required', 'duration' => 'required', 'description' => 'required|max:191', 'phone' => 'required', 'mission_id' => 'required', 'user_id' => 'required'];
            $validator = Validator::make(array('probability' => $request->probability, 'duration' => $request->duration, 'description' => $request->description, 'phone' => $request->phone, 'mission_id' => $request->mission_id, 'user_id' => $request->user_id), $rules);
            if ($validator->fails()) {
    ...
    public function add_notifcation()
    {
        ...
        $msg = array('title' => __('admin.new_project', [], 'en'), 'body' => 'put here content', 'image' => 'myIcon', /*Default Icon*/
            'sound' => 'mySound'
            /*Default sound*/
        );
        notify1($tokens, $msg);
    ...
    {
        $request = json_decode($request->getContent());
        $rules = ['lead_id' => 'required|max:191', 'location' => 'required|max:191', 'down_payment' => 'required|max:191', 'unit_type' => 'required|max:191', 'area_from' => 'required|numeric|min:0', 'area_to' => 'required|numeric|min:' . $request->area_from, 'price_from' => 'required|numeric|min:0', 'price_to' => 'required|numeric|min:' . $request->price_from, 'date' => 'required|max:191'];
        $validator = Validator::make(array('lead' => $request->lead, 'location' => $request->location, 'down_payment' => $request->down_payment, 'unit_type_id' => $request->unit_type_id, 'unit_type' => $request->unit_type, 'request_type' => $request->request_type, 'area_from' => $request->area_from, 'area_to' => $request->area_to, 'price_from' => $request->price_from, 'price_to' => $request->price_to, 'date' => $request->date), $rules);
        if ($validator->fails()) {
            $response = array('status' => 'error');
            return $response;
        } else {
            ...
            LogController::add_log(__('admin.created', [], 'ar') . ' ' . __('admin.request', [], 'ar'), __('admin.created', [], 'en') . ' ' . __('admin.request', [], 'en'), 'requests', $req->id, 'create', $request->user_id, $old_data);
    ...
    {
        $requests = json_decode($request->getContent());
        $rules = ['lead_id' => 'required', 'user_id' => 'required', 'title' => 'required', 'file' => 'required'];
        $validator = Validator::make(array('lead_id' => $request->lead_id, 'title' => $request->title, 'file' => $request->file, 'user_id' => $request->user_id, 'token' => $request->token), $rules);
        if ($validator->fails()) {
            return ['status' => false];
        } else {
            ...
            foreach ($leads as $row) {
                ...
                if ($row->other_phones != null) {
                    ...
                    foreach ($p as $key => $row1) {
                        foreach ($row1 as $key1 => $value) {
                            $value['whatsapp'];
                            array_push($other_phones, array('number' => $key1, 'whatsapp' => $value['whatsapp'], 'sms' => $value['sms'], 'viber' => $value['viber']));
                        }
                    }
                }
                if ($row->industry_id != null) {
                ...
                $lead = array('id' => $row->id, 'name' => $row->first_name . ' ' . $row->last_name, 'phone' => $row->phone, 'email' => $row->email ? $row->email : '', 'other_emails' => $other_emails, 'club' => $row->club, 'birth_date' => $row->birth_date ? date('d-m-Y', $row->birth_date) : '', 'other_phones' => $other_phones, 'company' => $row->company ? $row->company : '', 'school' => $row->school ? $row->school : '', 'image' => $image, 'notes' => $row->notes ? $row->notes : '', 'id_number' => $row->id_number ? $row->id_number : '', 'religion' => $row->religion ? $row->religion : '', 'address' => $row->address ? $row->address : '', 'country' => $country ? $country : '', 'social' => $social, 'title' => $title, 'industry' => $industry, 'agent_id' => $row->agent_id ? $row->agent_id : 0, 'agent_name' => @$row->agent->name,
                    // {{$user->photo ? $user->photo->file : 'http://placehold.it/400x400'}}
                    'commercial_agent_id' => $row->commercial_agent_id, 'commercial_agent_name' => @$row->commercialAgent->name, 'reference' => $row->reference ? $row->reference : '', 'lead_source' => @$source, 'probability' => $leadProbability);
                if ($get_full_info) {
    ...
    public function getFilterData(Request $request)
    {
        ...
        $locations = Location::select('id', $lang . '_name as name', 'lng' , 'lat', 'zoom')->get();
    ...
    public function getUnitFilterData(Request $request)
    {
        ...
        return ['unit_types' => $unitTypes, 'locations' => $locations, 'developers' => $developers, 'max_price' => $maxPrice, 'min_price' => $minPrice, 'max_downpayment' => $maxDown, 'min_downpayment' => $minDown, 'max_area' => $maxArea, 'min_area' => $minArea, 'resale_max_price' => $resaleMaxPirce, 'resale_min_price' => $resaleMinPirce, 'resale_max_area' => $resaleMaxArea, 'resale_min_area' => $resaleMinArea, 'rental_max_price' => $rentalMaxPirce, 'rental_min_price' => $rentalMinPirce, 'rental_max_area' => $rentalMaxArea, 'rental_min_area' => $rentalMinArea];
    }
    public function filterProjects(Request $request)
    {
        ...
        if (isset($request_json->installment) && $request_json->installment != "" && $request_json->installment > 0) {
            $projects = $projects->where('installment_year', $request_json->installment);
        }
        if (isset($request_json->developer) && $request_json->developer != "" && $request_json->developer > 0) {
        ...
        foreach ($projects as $row) {
            ...
            array_push($data, array('id' => $row->id, 'name' => $row->{$lang . '_name'}, 'price' => $row->meter_price, 'logo' => $row->logo, 'image' => $row->cover, 'payment' => $row->down_payment, 'lat' => $row->lat, 'lng' => $row->lng, 'zoom' => $row->zoom, 'installment_year' => $row->installment_year, 'delivery_date' => $row->delivery_date, 'location' => @$location));
        }
        if (count($data)) {
    ...
    public function fliterUnits(Request $request)
    {
        ...
        if ($type == 'resale') {
            ...
            foreach ($resaleUnit as $resale) {
                ...
                array_push($data, array('id' => $resale->id, 'location' => @$resale->locationData->{$lang . '_name'}, 'home_image' => $resale->image, 'other_images' => $allImages, 'title' => $resale->{$lang . '_title'}, 'price' => $resale->price, 'area' => $resale->area, 'rooms' => $resale->rooms, 'bathrooms' => $resale->bathrooms));
            }
        } else if ($type == 'rental') {
            ...
            foreach ($rentalUnit as $rental) {
                ...
                array_push($data, array('id' => $rental->id, 'location' => @$resale->locationData->{$lang . '_name'}, 'home_image' => $rental->image, 'other_images' => $allImages, 'title' => $rental->{$lang . '_title'}, 'price' => $rental->rent, 'area' => $rental->area, 'rooms' => $rental->rooms, 'bathrooms' => $rental->bathrooms));
            }
        }
        if (count($data)) {
            return ['status' => 'ok', 'projects' => $data];
    ...
    public function addUnit1(Request $r)
    {
        ...
        if ($request->privacy == 'team_only' || $request->privacy == 'only_me') {
    ...
    {
        $request_json = json_decode($request->getContent());
        $lead = Lead::where('phone', $request_json->phone)->first();
        $pros = '';
        if ($lead) {
            $full_info = $this->get_lead($request, $lead->id);
    ...
    public function get_lead(Request $request, $lead_id = 0)
    {
        ...
        if ($lead) {
            $lead->date=date('Y-m-d',$lead->date);
            ...
            $calls = Call::where('lead_id', $lead_id)->with('project')->with('call_status')->orderBy('id', 'desc')->with('user')->take(3)->get();
            $meetings = Meeting::where('lead_id', $lead_id)->with('project')->with('meeting_status')->orderBy('id', 'desc')->with('user')->take(3)->get();
            ...
            $requests = \App\Request::with('unittype')->with('loc')->where('lead_id', $lead_id)->orderBy('id', 'desc')->with('user')->take(3)->get();
            foreach ($requests as $req) {
                // if($req->projects == "[]")
                //     $req->projects = ["en_name"=>"", "ar_name"=>""];
            ...
            foreach ($meetings as $k => $meeting) {
                ...
                if($meeting->projects == "[]" || $meeting->projects == 'null' || $meeting->projects == 'NULL')
                    $meeting->projects = ["en_name"=>"", "ar_name"=>""];

                elseif(is_array(json_decode($meeting->projects))){
                    // dd(json_decode($meeting->projects));
                    foreach (json_decode($meeting->projects) as $key => $value) {
                        $project = @\App\Project::where('id', $value)->first();
                        array_push($arr, $project->en_name);
                    }
                }
                else
                {
                    $project = @\App\Project::where('id', $meeting->projects)->first();
                    array_push($arr, $project->en_name);
                }
                $meeting->projectsNameEn = $arr;
                $meetings[$k]->date=date('d-m-Y',$meetings[$k]->date);
            }
            foreach ($calls as $k => $call) {

                $arr = [];
                // dd(json_decode($meeting->projects));
                if($call->projects == "[]" || $call->projects == 'null' || $call->projects == 'NULL')
                    $call->projects = ["en_name"=>"", "ar_name"=>""];
                elseif(is_array(json_decode($call->projects))){
                ...
                {
                    $project = @\App\Project::where('id', $call->projects)->first();
                    if($project != null){
                        array_push($arr, $project->en_name);
                    }
                }
                $call->projectsNameEn = $arr;

                if (@$call->user->image == 'image.jpg') {
                    $calls[$k]->image = '';
                }
                if (!is_array($call->projects))
                    $proData = json_decode($call->projects);
                else
                    $proData = $call->projects;

                $projectsArray = [];
                if ($proData) {
                    if (!is_array($proData)) {
                        $projectsArray[] = $proData;
                    } else {
                        $projectsArray = $proData;
                    }
                }
                $call->project = [];
                if ($projectsArray) {
                    $call->project = @\App\Project::whereIn('id', $projectsArray)->get()->toArray();
                }

                /*if($call->projects == "[]") {$call->projects = ["en_name"=>"", "ar_name"=>""];}
                else {
                    $projsData = json_decode($call->projects,true);
                    if(is_array($projsData)){
                        if ($projsData) {
                            $call->project = [];
                            $call->project = @\App\Project::whereIn('id', $projsData)->get()->toArray();
                        }
                    }

                }*/

                $calls[$k]->date=date('d-m-Y',$calls[$k]->date);
            }
            ...
            $data = ['id' => $lead->id,
                ...
                'r_agent' => ['name' => @$lead->agent->name,
                'image' => @$lead->agent->image,
                'type' => @$lead->agent->agentType->name],
                'c_agent' => ['name' => @$lead->commercialAgent->name,
    ...
    public function downloadLead(Request $request)
    {
        ...
        foreach ($leads as $row) {
            ...
            //         $row->image = '';
            //     }
            //     if (@$row->agent->image == 'image.jpg') {
            //         @$row->agent->image = '';
            //     }
            //     if (@$row->commercialAgent->image == 'image.jpg') {
            //         @$row->commercialAgent->image = '';
            //     }
            ...
            $dataa = ['id' => $row->id, 'first_name' => $row->first_name, 'last_name' => $row->last_name, 'image' => $row->image, 'phone' => $row->phone, 'lead_source' => @$row->source->name, 'reference' => $row->reference, 'title' => @$row->title->name, 'industry' => @$row->industry->name, 'email' => $row->email, 'status' => $seen, 'created_by' => $row->user->name, 'created_at' => @$row->created_at->format('Y-m-d H:i:s'), 'updated_at' => @$row->updated_at->format('Y-m-d H:i:s'), 'r_agent' => ['name' => @$row->agent->name, 'image' => @$row->agent->image, 'type' => @$row->agent->agentType->name], 'c_agent' => ['name' => @$row->commercialAgent->name, 'image' => @$row->commercialAgent->image, 'type' => @$row->commercialAgent->agentType->name], 'calls' => $calls, 'meetings' => $meetings, 'voice_notes' => $voice_notes, 'notes' => $notes, 'documents' => @$row->documents];
            // //             /*return ['status' => 'ok', 'lead' => $data];
            // //         } else {
            // //             return ['status' => 'not_found'];
            // //         }*/
            // //         ////////////////////////////////////////////
            array_push($data, array('id' => $row->id, 'name' => $row->first_name . ' ' . $row->last_name, 'phone' => $row->phone, 'email' => $row->email ? $row->email : '', 'other_emails' => $other_emails, 'club' => $row->club, 'birth_date' => $row->birth_date ? date('d-m-Y', $row->birth_date) : '', 'other_phones' => $other_phones, 'company' => $row->company ? $row->company : '', 'school' => $row->school ? $row->school : '', 'image' => $image, 'notes' => $row->notes ? $row->notes : '', 'id_number' => $row->id_number ? $row->id_number : '', 'religion' => $row->religion ? $row->religion : '', 'address' => $row->address ? $row->address : '', 'country' => $country ? $country : '', 'social' => $social, 'title' => $title, 'industry' => $industry, 'agent_id' => '', 'agent_name' => '', 'data' => $dataa));
        }
        return $data;
    }
    //////////////HR  AgentApi///////////////
    ...
    {
        $request = json_decode($request->getContent());
        $rules = ['email' => 'required|max:191', 'name' => 'required|max:191', 'phone' => 'required|max:191'];
        $validator = Validator::make(array('email' => $request->email, 'name' => $request->name, 'phone' => $request->phone), $rules);
        if ($validator->fails()) {
            $response = array('status' => 'error');
            return $response;
        } else {
    ...
    public function checkEmployees(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        $validator->SetAttributeNames(['latitude' => trans('admin.latitude'), 'longitude' => trans('admin.longitude')]);
        if ($validator->fails()) {
            return back()->withInput()->withErrors($validator);
        } else {
            $request = json_decode($request->getContent());
            ...
            if ($request->user_id) {
                if ($request->check_in) {
                    ...
                    $attend->employee_id = $request->user_id;
                    $attend->check_in = $request->check_in;
                    $attend->date = date('Y-m-d', strtotime($request->check_in));
                    ...
                    $attend = new MobileAttendance();
                    $attend->employee_id = $request->user_id;
                    $attend->check_out = $request->check_out;
                    $attend->date = date('Y-m-d', strtotime($request->check_out));
                    $attend->latitude = $request->latitude;
                    $attend->longitude = $request->longitude;
                    $attend->save;
                }
                $app_attend = new AttendanceApproved();
                $app_attend->approved_emp = $user_id;
                $app_attend->check_employee = $request->check_in ? "in" : "out";
                $app_attend->latitude = $request->latitude;
                $app_attend->longitude = $request->longitude;
                $app_attend->mobile_attendance_id = $attend->id;
                $app_attend->save();
                if ($request->check_in) {
                    $attend->check_in = date('h:i:s', strtotime($request->check_in));
                } else {
                    $attend->check_out = date('h:i:s', strtotime($request->check_out));
                }
                $attend->save();
                return ['status' => 'ok'];
            } else {
    ...
    public function approveAttend(Request $request)
    {
        ...
        if (User::Find($request->user_id)->type == 'admin' || Employee::where('user_id', $request->user_id)->first()->is_hr == 1) {
            $app_attend = AttendanceApproved::where('mobile_attendance_id', $request->mobile_attendance_id);
            $app_attend->hr_emp = $request->user_id;
            $app_attend->date = $request->date;
            $app_attend->is_approve = $request->is_approved;
            $app_attend->save();
            $attend = MobileAttendance::where('id', $request->mobile_attendance_id)->first();
    ...
    public function vacationRequests(Request $request)
    {
        if ($request->user_id) {
            ...
            $emp_name = Employee::find($emp)->en_first_name . ' ' . Employee::find($emp)->en_last_name;
            $emp_photo = Photo::where('employee_id', Employee::where('user_id', $user_id)->first())->where('code', 'profile')->first();
            $emp_title = Employee::where('user_id', $user_id)->pluck('job_title_id');
            $job_title = JobTitle::find($emp_title)->pluck('en_name');
            $vacations = Vacation::where('employee_id', $emp)->where('type', 'request')->get();
            $data = [];
            foreach ($vacations as $vacation) {
                $data = ['emp' => ['name' => $emp_name, 'job title' => $job_title, 'image' => $emp_photo], 'vacations' => ['type' => $vacation->reason, 'number of Days' => $vacation->number_of_days, 'from date' => $vacation->start_date, 'to date' => $vacation->end_date]];
            }
            return ['lead' => $data, 'status' => 'ok'];
        } else {
            return ['status' => 'not_found'];
        }
    }
    ...
    {
        $request = json_decode($request->getContent());
        $rules = ['vacations_types_id' => 'required', 'start_date' => 'required', 'end_date' => 'required', 'vacation_payment' => 'required'];
        $validator = Validator::make(array('vacations_types_id' => $request->vacations_types_id, 'start_date' => $request->start_date, 'end_date' => $request->end_date, 'vacation_payment' => $request->vacation_payment), $rules);
        if ($validator->fails()) {
            $response = array('status' => 'error');
            return $response;
        } else {
            if ($request->user_id) {
                ...
                $vacation->employee_id = $request->user_id;
    ...
    {
        $request = json_decode($request->getContent());
        $rules = ['vacation_id' => 'required', 'approved_date' => 'required', 'is_approved' => 'required'];
        $validator = Validator::make($request->all(), $rules);
        $validator->SetAttributeNames(['vacation_id' => trans('admin.vacation_type_id'), 'approved_date' => trans('admin.approved_date'), 'is_approved' => trans('admin.vacation_pay')]);
        if ($validator->fails()) {
            return back()->withInput()->withErrors($validator);
        } else {
            if (User::Find($request->user_id)->type == 'admin' || Employee::where('user_id', $request->user_id)->first()->is_hr == 1) {
                $app_vacation = Vacation::findorfail($request->vacation_id);
                $app_vacation->date = $request->date;
                $app_vacation->is_approve = $request->is_approved;
                $app_vacation->approved_by = $request->user_id;
    ...
    public function newProfile(Request $request)
    {
        ...
        if ($request->user_id) {
            ...
            $commision = $com + $sub;
            $vacations = Vacation::where('employee_id', $emp)->where('type', 'request')->get();
            $data = array('name' => $name, 'deal_progress' => $proposal, 'commission' => $commision, 'month_target' => $mtarget, 'year_target' => $ytarget, 'percent' => $percent, 'vacations' => $vacations);
    ...
    public function vacation(Request $request)
    {
        ...
        if ($request->user_id) {
            ...
            foreach ($vacations as $vacation) {
                ...
                // $vac = VacationType::where('id',$vacation->vacation_type_id)->first()->name;
                // $v = $vac->name;
                array_push($data,
                    array('title' => 'Request Vacation',
                        'type' => 'vacation',
                        'name' => $name,
                        'image' => $image ? $image : "",
                        ...
                        'vacation_limt' => $vacation->number_of_days,
            ...
            foreach ($attends as $attend) {
                ...
                array_push($data, array('title' => 'Request Attendance', 'type' => 'attendance', 'name' => $name ? $name : "", 'image' => $image ? $image : "", 'mobile_attendance_id' => $attend->id ? $attend->id : "", 'employee_id' => $attend->employee_id ? $attend->employee_id : "", 'approval' => $attend->is_approved ? $attend->is_approved : -1, 'longitude' => $attend->longitude ? $attend->longitude : "", 'latitude' => $attend->latitude ? $attend->latitude : "", 'check_in' => $attend->check_in ? $attend->check_in : "", 'check_out' => $attend->check_out ? $attend->check_out : "", 'date' => $attend->date ? $attend->date : "", 'permmison' => User::find($user_id)->type == "admin" ? "true" : "false"));
            }
    ...
    {
        $request = json_decode($request->getContent());
        $rules = ['user_id' => 'required', 'player_id' => 'required'];
        $validator = Validator::make(array('user_id' => $request->user_id, 'player_id' => $request->player_id), $rules);
        if ($validator->fails()) {
            $response = array('status' => 'error');
            return $response;
        }{
            if (count(Player::where('player_id', $request->player_id)->get()) < 1) {
                ...
                Player::where('player_id', $request->player_id)->update(['user_id' => $request->user_id]);
                echo 2;
            }
        }
    }
    public function ResaleOfMe(Request $request){
        $meResaleUnit = ResaleUnit::whereHas('lead',function($q) use ($request){
            $q->where('agent_id',$request->user_id)->orWhere('commercial_agent_id',$request->user_id);
        })->get();
        return $meResaleUnit;
    }
    public function ResaleOfOthers(Request $request){
        if(User::find($request->user_id)->type=="admin"){
            ...
            $oResaleUnit = ResaleUnit::whereHas('lead',function($q) use ($users,$request){
            ...
            $leader=\App\Group::where('team_leader_id',$request->user_id)->first()->id;
            $groupMembers = \App\GroupMember::where('group_id', $leader)->get();
            $users = $groupMembers->pluck('member_id')->toArray();
            $oResaleUnit = ResaleUnit::where('privacy','public')->orWhere(function ($q) use ($users,$request){
                $q->whereHas('lead',function($q) use ($users){
                $q->whereIn('agent_id',$users)->orWhereIn('commercial_agent_id',$users);
            })->where('privacy','!=','only_me');})->get();
        }else{
            $group_id=\App\GroupMember::where('member_id',$request->user_id)->first();
    ...
    public function RentalOfMe(Request $request){
        $meRentalUnit = RentalUnit::whereHas('lead',function($q) use ($request){
            $q->where('agent_id',$request->user_id)->orWhere('commercial_agent_id',$request->user_id);
        })->get();
        return $meRentalUnit;
    }
    public function RentalOfOthers(Request $request){
        if(User::find($request->user_id)->type=="admin"){
            $groups = \App\Group::get()->pluck('team_leader_id')->toArray();
            $groupMembers = \App\GroupMember::get();
            $users = $groupMembers->pluck('member_id')->toArray();
            array_push($users,$groups);
            $oRentalUnit = RentalUnit::whereHas('lead',function($q) use ($users,$request){
                $q->whereIn('agent_id',$users)->orWhereIn('commercial_agent_id',$users);
            })->get();
        }elseif(\App\Group::where('team_leader_id',$request->user_id)->count()>0){
            $leader=\App\Group::where('team_leader_id',$request->user_id)->first()->id;
            $groupMembers = \App\GroupMember::where('group_id', $leader)->get();
            $users = $groupMembers->pluck('member_id')->toArray();
            $oRentalUnit = RentalUnit::where('privacy','public')->orWhere(function ($q) use ($users,$request){
                $q->whereHas('lead',function($q) use ($users){
                $q->whereIn('agent_id',$users)->orWhereIn('commercial_agent_id',$users);
            })->where('privacy','!=','only_me');})->get();
        }else{
    ...
    public function fav_lead(Request $request)
    {
        ...
        return ['data' => $lead->favorite, 'status' => 'ok'];
    }
    ...
    public function hot_lead(Request $request)
    {
        ...
        return ['data' => $lead->hot, 'status' => 'ok'];
    }

    public function showLeadAction($type,$id)
    {
        switch ($type) {
            case 'call':
                $call = @\App\Call::where('id',$id)->with('call_status')->with('project')->first();
                return $call;
            break;
    ...
    public function updateLeadAction(Request $request,$type,$id)
    {
        switch ($type) {
            case 'call':
            ...
            case 'meeting':
                ...
                if($meeting){
                    ...
                    $meeting->meeting_in_out = $request->meeting_in_out;
                    $meeting->save();
    ...
    public function deleteLeadAction($type,$id)
    {
        switch ($type) {
            case 'call':
                $call = @\App\Call::find($id);
