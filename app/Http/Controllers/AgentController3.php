...
class AgentController3 extends Controller
{
    ...
    public function send_cil(Request $request)
    {
        ...
        $project = 0;
        $developerErrors=array();
        if($lead){
            foreach ($request->cil as $k => $row) {
                $developer = @Developer::find($row->developer);
                if ($row->developer && $row->project) {
            ...
            $response = array('status' => 'error','mes'=>"lead not found.");
            return $response;
        }
        if(count($developerErrors)>0){
        $response = array('status' => 'error','elements'=>$developerErrors);
        return $response;
        }
        ...
        if ($validator->fails()) {
            ...
            foreach ($users as $user) {
                foreach ($user->player as $p) {
                    OneSignal::sendNotificationToUser($message, $p->player_id, $url, $data = null, $buttons = null, $schedule = null);
                }
            }
            $old_data = json_encode($req);
            LogController::add_log(
    ...
    public function notification(Request $request)
    {
        ...
        // $gitminus = date('Y-m-dT H:i:s A', strtotime($request->time) - 10);
        // dd($gitminus);
        $request     = json_decode($request->getContent());
        $switch      = AdminNotification::where('assigned_to', $request->user_id)->where('type', 'switch')->orderBy('created_at', 'desc')->take(10)->get();
        $notes       = AdminNotification::where('assigned_to', $request->user_id)->where('type', 'note_lead')->orderBy('created_at', 'desc')->take(10)->get();
        $broadCosts  = AdminNotification::where('assigned_to', $request->user_id)->where('type', 'broadcast')->orderBy('created_at', 'desc')->take(10)->get();
        ...
        $meeting     = AdminNotification::where('assigned_to', $request->user_id)->where('type', 'meeting')->get();
        $calls       = AdminNotification::where('assigned_to', $request->user_id)->where('type', 'call')->get();
        $cils        = AdminNotification::where('assigned_to', $request->user_id)->where('type', 'cil')->get();
        ...
        'type' => $type,
        'title' => "BroadCast Request Is Added ...",
        'id' => @$lead->id,
        ...
        foreach ($switch as $row) {
            ...
            array_push($data, array(
                ...
                'title'        => User::find($row->user_id)->name . ' switch  ' . @$lead->first_name . ' ' . @$lead->last_name . ' to you',
                'id'           => @$lead->id,
                'name'         => @$lead->first_name . ' ' . @$lead->last_name,
                'phone'        => @$lead->phone,
                'email'        => @$lead->email ? $lead->email : '',
                ...
                'date'         => $date,
                'time'         => $time,
                'diff'         => @$lead->created_at ? @$lead->created_at->diffForHumans() : '',
                'notiy_id'     => $row->id,
            ));
        }
        ...
        foreach ($meeting as $row) {
            ...
            array_push($data, array(
                'type'         => $type,
                'notificationType'         => "note_lead",
                'title'        => User::find($row->user_id)->name . 'Have a meeting with"' . ' ' . @$lead->first_name . ' ' . @$lead->last_name . '"',
                'id'           => @$lead->id,
                'name'         => @$lead->first_name . ' ' . @$lead->last_name,
                'phone'        => @$lead->phone,
                'email'        => @$lead->email ? $lead->email : '',
                ...
                'social'       => $social,
                'industry'     => $industry,
                'icon'         => 'notify/notes.png',
                'status'       => $row->status,
                'date'         => $date,
                'time'         => $time,
                'diff'         => $row->created_at ? $row->created_at->diffForHumans() : '',
                'notiy_id'     => $row->id,
            ));
        }
        ...
        foreach ($calls as $row) {
            ...
            array_push($data, array(
                'type'         => $type,
                'notificationType'         => "note_lead",
                'title'        => User::find($row->user_id)->name . 'Have a call  with"' . ' ' . @$lead->first_name . ' ' . @$lead->last_name . '"',
                'id'           => @$lead->id,
                'name'         => @$lead->first_name . ' ' . @$lead->last_name,
                'phone'        => @$lead->phone,
                'email'        => @$lead->email ? $lead->email : '',
        ...
        foreach ($cils as $row) {
            ...
            array_push($data, array(
                'type'         => $type,
                'notificationType'         => "note_lead",
                'title'        => User::find($row->user_id)->name . 'Sent cils with"' . ' ' . @$lead->first_name . ' ' . @$lead->last_name . '"',
                'id'           => @$lead->id,
                'name'         => @$lead->first_name . ' ' . @$lead->last_name,
                'phone'        => @$lead->phone,
                'email'        => @$lead->email ? $lead->email : '',
        ...
        foreach ($notes as $row) {
            ...
            array_push($data, array(
                'type'         => $type,
                'notificationType'         => "note_lead",
                'title'        => User::find($row->user_id)->name . ' add a note in lead"' . ' ' . @$lead->first_name . ' ' . @$lead->last_name . '" to you',
                'id'           => @$lead->id,
                'name'         => @$lead->first_name . ' ' . @$lead->last_name,
                'phone'        => @$lead->phone,
                'email'        => @$lead->email ? $lead->email : '',
        ...
        foreach ($added_lead as $row) {
            ...
            array_push($data, array(
                'type'         => $type,
                'notificationType'         => "added_lead",
                'title'        => @$lead->first_name . ' ' . @$lead->last_name . 'Added  by ' . User::find($row->user_id)->name,
                'id'           => @$lead->id,
                'name'         => @$lead->first_name . ' ' . @$lead->last_name,
                'phone'        => @$lead->phone,
                'email'        => @$lead->email ? $lead->email : '',
    ...
    public function notificationsChangeStatus($id)
    {
        $cahange_status = AdminNotification::find($id);
        $cahange_status->status = 1;
        $cahange_status->update();
        $num = AdminNotification::where('status',0)->count();
        return response()->json($num);
    }

    public function ChangeAllNotification(){
        $cahange_status = AdminNotification::where('status',0)->get();
        foreach( $cahange_status as $status){
            $status->status = 1;
            $status->update();
        }
        $num = AdminNotification::where('status',0)->count();
    ...
    public function clearNumCil(){
        $cahange_status = Ciil::where('seen',0)->select('id','seen')->get();
        foreach( $cahange_status as $status){
            $status->seen = 1;
            $status->update();
        }
        $num = Ciil::where('seen',0)->count();
        return response()->json($num);
    }
    ...
    public function add_image(Request $request)
    {
        ...
        $type= $request['type'];
            ...
            $set = Setting::first();
            if ($request['main'] && isset($unit)) {

                $path = upload($request['main'], $route);

                if ($path == 'invalid') {

                    return json_encode(['status' => false,/*  */ 'msg' => 'File is invalid']);
                }
                $unit->image = $path;
                // works locally
                ...
                return json_encode(['status' => false, 'msg' => 'error']);
            }
            if ($request['image'] && isset($unit)) {
                ...
                foreach ($request['image'] as $img) {
                    ...
                    if ($other_image == 'invalid') {
                        return ['status' => false, 'msg' => 'File is invalid'];
                    }
                    // works locally
                    /**
                    $watermark = Image::make('uploads/' . $set->watermark)->resize(50, 50);
