...
class ReceivedprojectController extends Controller
{
    ...
    public function accept($id)
    {
        ...
        $data = ['project_id' => @$re->project_id, 'token' => '$2y$10$aJOEOQOg86C3/zRki8uMK.npVan9iGDUty0r4YSmu8oFhdEtUjWhO'];
        ...
        if (@Project::where('portal_id', @$data->projects->id)->count() == 0) {
            ...
            if ($data->developer) {
                ...
                if ((null != $row->logo || '' != $row->logo) && $developerLogo == null) {
                    $image = Image::make($filename);
                    $ext = explode('.', $filename);
                    $name = 'logo_' . rand(0, 99999999999) . '.' . end($ext);
                    $image->save("uploads/" . $name);
                    $developer->logo = $name;
                }
                else if((null != $row->logo || '' != $row->logo) && $developerLogo != null){
                    $file_path = 'uploads/' . $developerLogo;
                    @unlink($file_path);
                    $image = Image::make($filename);
                    $ext = explode('.', $filename);
                    $name = 'logo_' . rand(0, 99999999999) . '.' . end($ext);
            ...
            if ($data->location) {
                ...
                if (null != $row->map_marker || '' != $row->map_marker) {
            ...
            if (@$data->location) {
                ...
                if ("[]" != $row->developer_pdf && "" != $row->developer_pdf) {
                    $pdfs = json_decode($row->developer_pdf);
                    $pdfArr = [];
                    if ($pdfs) {
                        foreach ($pdfs as $pdf) {
                ...
                if ("[]" != $row->broker_pdf && "" != $row->broker_pdf) {
                    $pdfs = json_decode($row->broker_pdf);
                    $pdfArr = [];
                    if ($pdfs) {
                        foreach ($pdfs as $pdf) {
