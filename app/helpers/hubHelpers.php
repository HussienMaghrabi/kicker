...
function decryptHelper($encryptedString)
{
    $pss = null;
    try {
        $pss = decrypt($encryptedString);
    } catch (Exception $e) {
...
function phoneNumbersHandler($phone,$countries){
    $phoneWithNoDecorations = preg_replace('~\D~', '', $phone);     // remove Non numeric characters
    $phoneWithNoDecorations = preg_replace('/^0+/','', $phoneWithNoDecorations);   //remove leading zeros
    $countryIso = '';
    $response = array();
    if(strlen($phoneWithNoDecorations) > 2){
        if($phoneWithNoDecorations[0] == '1' && ($phoneWithNoDecorations[1] == '0' || $phoneWithNoDecorations[1] == '1' ||
            $phoneWithNoDecorations[1] == '2' || $phoneWithNoDecorations[1] == '5' ))
        {
            $countryIso = "EG"; 
        }
        else{
            foreach($countries as $country){
                $temp = $phoneWithNoDecorations;
                $phoneWithNoDecorations = preg_replace('/^'.$country->code.'+/','', $phoneWithNoDecorations);
                if($phoneWithNoDecorations != $temp){
                    $countryIso = $country->iso;
                    break;
                }
            }
            $phoneWithNoDecorations = preg_replace('/^0+/','', $phoneWithNoDecorations);   //remove leading zeros
            if($countryIso == '' || $phoneWithNoDecorations == '' || strlen($phoneWithNoDecorations) < 2){
